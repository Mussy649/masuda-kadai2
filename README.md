...
purchases
```
アプリケーションで使用するテーブルは上記8テーブルです。

これにLaravelがマイグレーション履歴を管理する`migrations`テーブルを加え、データベース全体では9テーブルで構成されています。

本アプリでは使用しないため、以下の機能および関連テーブルは作成していません。
...
そのため、`password_resets`、`failed_jobs`、`personal_access_tokens`および2要素認証用カラムは使用していません。
...
購入画面の配送先変更画面で入力した住所は、購入対象商品ごとの配送先としてセッションへ一時保存します。

この操作では、`users`テーブルに登録されているプロフィール住所は変更されません。

購入成立時には、変更後の配送先情報を`purchases`テーブルへ保存します。配送先を変更していない場合は、プロフィールに登録されている住所を使用します。

`purchases`テーブルへ保存された配送先情報は購入時点の情報として保持されるため、その後プロフィール住所を変更しても更新されません。
