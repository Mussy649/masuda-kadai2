...
「コンビニ払い」を選択して「購入する」ボタンを押すと、Stripeのコンビニ決済画面へ遷移します。

本模擬案件では処理を簡略化し、Stripeのコンビニ決済画面へ正常に遷移した時点で購入成立として扱います。

Stripeのコンビニ決済画面にはアプリへ戻るボタンがないため、購入商品一覧を確認する場合は、ブラウザで以下のURLを開き直してください。
